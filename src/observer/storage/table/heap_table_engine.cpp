...
RC HeapTableEngine::create_index(Trx *trx, const vector<const FieldMeta *> &field_metas, const char *index_name, bool unique)
{
  if (common::is_blank(index_name) || field_metas.empty()) {
    LOG_INFO("Invalid input arguments, table name is %s, index_name is blank or field list is empty", table_meta_->name());
  ...
  rc = index->create(table_, index_file.c_str(), new_index_meta, fields);
  if (rc != RC::SUCCESS) {
    delete index;
